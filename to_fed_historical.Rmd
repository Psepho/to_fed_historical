...
```{r pred_candidates, cache=TRUE, echo=FALSE, warning=FALSE}
pred_candidates <- readxl::read_excel("data/2015 Candidates.xlsx", skip = 1)[,c(2, 4:6)]
names(pred_candidates) <- c("district", "party", "incumbent", "star_candidate")
...
spread_star_candidate <- pred_candidates %>%
  dplyr::select(district, party, star_candidate) %>%
...
spread_incumbent <- pred_candidates %>%
  dplyr::select(district, party, incumbent) %>%
...
```{r toronto_prediction, echo=FALSE, warning=FALSE}
...
ggplot(map_results, aes(map_id = district)) +
  ...
  scale_fill_brewer("Proportion of votes", 
                    ...
                    type = "seq", palette = "YlOrBr") +
  labs(x = "", y = "", title = paste0(year, " Predictions for the 2015 Federal General Election")) +
  theme(axis.ticks.y = element_blank(), axis.text.y = element_blank(), # get rid of x ticks/text
        axis.ticks.x = element_blank(), axis.text.x = element_blank(), # get rid of y ticks/text
        plot.title = element_text(lineheight=.8, face="bold", vjust=1)) + # make title bold and add space
  expand_limits(x = geo$long, y = geo$lat) +

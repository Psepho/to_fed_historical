...
```{r change_in_votes, echo=FALSE, message=FALSE}
...
```{r incumbents, echo=FALSE, message=FALSE}
...
```{r leaders, echo=FALSE, message=FALSE}
...
Now that we've looked at aspects of candidates and their leaders, we can move to attributes of the voters themselves. In this case, we use data from the 2011 Census, specifically age, gender, and family income.
...
demog_model <- lme(prop_votes ~ party * (gender + age + family_income), data = data, 
                    random = ~1|year/Geo_Code, na.action = na.exclude)
summary(demog_model)
```
The linear, mixed effects model shows that the relationships among age, gender, family income, and the proportion of votes vary widely across the parties (as expected). In general, the effects are:

- Age tends to increase support for Conservatives and Liberals (though the slope is higher for Conservatives; 0.010 vs 0.007), while decreasing support for NDP (-0.011).
- Family income tends to increase support for Conservatives (0.05/$10,000) while decreasing for the other two major parties by roughly the same magnitude.
- Gender is the strange one. We're getting a really strong signal for increased support of women for Conservatives, moderate support for Liberals, and strong negative support for NDP. I would have guessed the opposite. It might be that the combination of age, gender, and family income "explains" this effect (i.e., lots of old, rich women tend to vote Conservative), but this seems unlikely. I'm going to keep looking into this.
...
comprehensive_model <- lme(prop_votes ~ party * (gender + age + family_income) 
                           + likeability + star_candidate*incumbent, data = data, 
                    random = ~1|year/Geo_Code, na.action = na.exclude)
